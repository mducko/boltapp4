...
import { checkStorageQuotas, cleanupStorage, type StorageStats } from './StorageQuotas';
...
export class StorageManager {
  ...
  async get<T>(key: string, options: StorageOptions = {}): Promise<T | null> {
    try {
      ...
      if (options.encrypt) {
        data = await this.secureStorage.decrypt(data);
      }
  ...
  async set<T>(key: string, value: T, options: StorageOptions = {}): Promise<void> {
    try {
      ...
      // Handle encryption
      if (options.encrypt) {
        data = await this.secureStorage.encrypt(data);
      }
  ...
  async getStorageStats(): Promise<StorageStats> {
    return await checkStorageQuotas().then(result => result.stats);
  }
